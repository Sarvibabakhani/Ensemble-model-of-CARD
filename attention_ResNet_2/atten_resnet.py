...
def residual_unit(residual_input_data, filters):
  ...
  conv_op_2 = Conv2D(filters=filter2,kernel_size=3, strides=1,padding='same')(activation_op_2)
  ...
  if identity_x.shape[-1] != conv_op_3.shape[-1]:
    ...
    identity_x = Conv2D(filters=filter_n,kernel_size=1, strides=1,padding='same')(identity_x)
...
def atten_resnet(input_shape,n_classes):
  ...
  res_unit_1 = residual_unit(max_pool_layer_1, filters=[16, 16, 64])
  att_mod_1 = attention_module_1(res_unit_1, filters=[16, 16, 64])
        
  # Residual Unit then Attention Module #2
  res_unit_2 = residual_unit(att_mod_1, filters=[32, 32, 128])
  att_mod_2 = attention_module_2(res_unit_2, filters=[32, 32, 128])

  # # Residual Unit then Attention Module #3
  res_unit_3 = residual_unit(att_mod_2, filters=[64, 64, 256])
  att_mod_3 = attention_module_3(res_unit_3, filters=[64, 64, 256])

  # # Ending it all
  res_unit_end_1 = residual_unit(att_mod_3, filters=[128, 128, 512])
  res_unit_end_2 = residual_unit(res_unit_end_1, filters=[128, 128, 512])
  res_unit_end_3 = residual_unit(res_unit_end_2, filters=[128, 128, 512])
  res_unit_end_4 = residual_unit(res_unit_end_3, filters=[128, 128, 512])
